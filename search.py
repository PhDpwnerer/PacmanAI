...
def breadthFirstSearch(problem, state):
    """
    ...
    for limit in range(1, limitCap): 
            ...
            while (not frontier.isEmpty()):
                ...
                if problem.goalTest(node.state):
                    result = actionList(node, problem)
                    print(result) 
                    return result
                explored.add(node)
                if nodeLevel(node) < limit:
                    actions = problem.getActions(node.state)
                    for action in actions:
                        child = Node(problem.getResult(node.state, action), node, action, 1)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    frontier.push(Node(currentState, None, None, 0), heuristic(currentState, problem))
    while (not frontier.isEmpty()):
        node = frontier.pop()
        if problem.goalTest(node.state):
            result = actionList(node, problem)
        ...
        actions = problem.getActions(node.state)
        for action in actions:
            child = Node(problem.getResult(node.state, action), node, action, node.path_cost+problem.getCost(node.state, action))
