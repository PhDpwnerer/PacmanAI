...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.startingState = (self.startingPosition, 1, 1, 1, 1) #the four ones signify whether the corresponding corner food was eaten
...
def cornersHeuristic(state, problem):
    ...
    for i in range(0,4):
        if state[i+1] == 1:
            distance = manhattanDistance(pos, corners[i])
            ranking = (distance, corners[i])
            objectives.append(ranking)
    while len(objectives) != 0:
        objectives.sort()
        distance, corner = objectives.pop(0)
        result += distance
        for i in range(len(objectives)):
            distance2, corner2 = objectives[i]
            objectives[i] = (manhattanDistance(corner2, corner), corner2)
    return result
...
def foodHeuristic(state, problem):
    ...
    numFood = foodGrid.count()
    result = 0
    objectives = []
    for x in range(len(foodGrid.data)):
        for y in range(len(foodGrid[x])):
            if (foodGrid[x][y]):
                # result += 0
                objectives.append((manhattanDistance(position, (x,y)), (x,y)))
    
    if len(objectives) != 0:
        objectives.sort()
        farthest = objectives[-1]
        closest = objectives[0]
        distance2, pos2 = closest
        distance1, pos1 = farthest
        result += max(distance1, foodGrid.count()+distance2-1)
    return result
